...
void lireFichierCSV(const char *nomFichier, pavl* racine) {
    ...
    while (fgets(ligne, sizeof(ligne), f)) {
        //karen change le s la et reflichis comment faire pour continue!!
         // Sauter les lignes d'entête du fichier CSV
        // On suppose que les entêtes commencent par 'S', à ajuster si nécessaire
        if (line[0] == 'S') {
            continue; // Ignorer cette ligne et passer à la suivante
        }
         // Utiliser sscanf pour extraire les informations de la ligne
        // On suppose que le fichier CSV est séparé par des points-virgules
...
// Fonction pour comparer deux stations par leur capacité
int comparerParCapacite(Station* a, Station* b) {
    return a->capacite - b->capacite; // Retourne la différence des capacités
}

// Fonction pour trier les stations par capacité (tri à bulles)
void trierStationsParCapacite(Station** stations, int taille) {
    for (int i = 0; i < taille - 1; i++) {
        for (int j = 0; j < taille - i - 1; j++) {
            if (comparerParCapacite(stations[j], stations[j + 1]) > 0) {
                // Échange des positions si les capacités sont dans le mauvais ordre
                Station* temp = stations[j];
...
void afficherStations(Station** stations, int taille) {
    for (int i = 0; i < taille; i++) {
        printf("Station ID: %d | Capacite: %d | Load: %d | Centrale ID: %d\n",
               stations[i]->id, stations[i]->capacite, stations[i]->load, stations[i]->centrale_id);
    }
}
